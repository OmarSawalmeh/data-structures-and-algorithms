...
function transformToLis(obj){
  ...
  if(Object.keys(obj)[0] == "name"){
    let name = `<li>name: bob</li>`;
    let age = `<li>age: 32</li>`; 
    let list = [name, age];
    return list;
  }
  else{
...
for(let i=0; i<input.length; i++){
  ...
  for(let j=0; j<input[i].length; j++){
    if(input[i][j]%5 == 0  & typeof input[i][j] == "number"){
      curArr.push(Math.pow(2, input[i][j]));
    }
  }
  newArr.push(curArr);
}
...
let allName = ""
  ...
  if(value.gender == 'female' || value.gender == 'male'){
    allName += value.name + " and ";
  }
})

allName = allName.slice(0, -5);
return allName
};
...
let findShortest = (data) => {
  // Solution code here...
let val = parseInt(data[0].height);
let name;
