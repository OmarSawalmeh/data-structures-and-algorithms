...
const sortStarWarsCharacters = (starWarsArr) => {
  // Solution code here...
  let newArrObj = [];
   newArrObj.push(starWarsArr[0]);
  newArrObj.push(starWarsArr[2]);
  starWarsArr.forEach((value, index)=>{
    if(value.name.length > newArrObj[0].name.length){
      newArrObj.unshift(value);
    }
  })
